...
const stateManager = new StateManager<typeof defaultStorageObject>(loadState, saveState, {
	delay: 500,
	maxWait: 1000
});
...
(async () => {
	try {
		...
		getStorageArea().set(cleanObject(stateManager.state!, defaultStorageObject));
	} catch (error) {
...
async function loadState(): Promise<typeof defaultStorageObject> {
	try {
		const raw = (await getStorageArea().get()) as Partial<typeof defaultStorageObject>;
