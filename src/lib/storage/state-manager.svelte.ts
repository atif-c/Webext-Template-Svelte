 ...
 *
 *  * Source: {@link https://github.com/atif-c/Svelte-5-State-Manager GitHub}
 *
 *  Type constraints:
...
export class StateManager<T extends Record<string, unknown>> {
    state = $state<T>({} as T);
    private loadCallback: () => T | Promise<T>;
    private saveCallback?: (storage: T) => void | Promise<void>;
    private debounceOptions: {
        delay: number;
        maxWait: number;
        immediate?: boolean;
    };
