...
async function copyViteOutputTo(distDir) {
...
async function main() {
	...
	const results = await Promise.all(requestedTargets.map(t => buildTarget(t)));
...
(async () => {
	try {
		...
		console.log('Build completed successfully for:', results.map(r => r.target).join(', '));
		process.exitCode = 0;
	} catch (err) {
		console.error('Build failed:', err && err.stack ? err.stack : String(err));
		process.exitCode = 1;
	}
})();
